...
describe('formatPercent', () => {
  ...
  test('should trim trailing zeros when trimTrailingZeros is true', () => {
    expect(formatPercent(12.00, { trimTrailingZeros: true })).toBe('+12%');
    expect(formatPercent(12.30, { trimTrailingZeros: true })).toBe('+12.3%');
    expect(formatPercent(12.340, { trimTrailingZeros: true, decimals: 3 })).toBe('+12.34%');
  });
  ...
  test('should trim trailing zeros and show plus sign', () => {
    expect(formatPercent(10.0, { decimals: 0, trimTrailingZeros: true })).toBe('+10%');
    expect(formatPercent(10.5, { trimTrailingZeros: true, decimals: 1 })).toBe('+10.5%');
  });

  test('should trim trailing zeros and not show plus sign', () => {
    expect(formatPercent(10.0, { decimals: 0, trimTrailingZeros: true, showPlusSign: false })).toBe('10%');
    expect(formatPercent(10.5, { trimTrailingZeros: true, decimals: 1, showPlusSign: false })).toBe('10.5%');
  });
  ...
  test('should handle all options combined (positive number)', () => {
    const options = { decimals: 1, showPlusSign: false, trimTrailingZeros: true };
    expect(formatPercent(12.345, options)).toBe('12.3%');
    expect(formatPercent(12.000, options)).toBe('12%');
  });
