...
import { KlineMessage } from "../../../binance/types/kline";
import {
    Candle,
    FetchCandlesParams,
...
export const subscribeWS = createAsyncThunk<
    ...
    FetchCandlesParams,
    { state: RootState }
>("candles/subscribe", async ({ symbol, interval }, { getState, dispatch }) => {
    const streamName = `${symbol}_${interval}`;
    const { stream, onMessage } = registerStream<'kline'>('kline', symbol, { interval });
